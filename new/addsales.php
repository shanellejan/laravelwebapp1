	...
	<div id="header">
		...
		?>
			...
			<ul id="navigation">
				...
				<li    class="selected">
					<a href="addsales.php" title= "The Designer">Sales</a>
				</li>
 ...
 <!--Main Body-->
	...
	<select name = "no" style = "width: 90px; height: 35px; text-align: center;">
		...
		<?php
			...
			$nprice = $_POST['Price'];
			$ndate = $_POST['Stock_Delivery_Date'];
			$invdate = date('Y/m/d'); //current date
			$user = $_SESSION['User'];
			
			if(isset($_POST['save'])){
				$sql = mysql_query("INSERT INTO stocks_inventory
				(Product_No, New_Delivered_Stocks, Price, Stock_Delivery_Date, Inventory_Date) VALUES
				('$getNo', '$new', '$nprice', '$ndate', '$invdate')");
